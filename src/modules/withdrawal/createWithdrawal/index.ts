...
type Params<M extends SkybridgeMode> = Pick<
  ...
  'context' | 'addressUserIn' | 'currencyOut' | 'amountUser'
>;
...
const createSwapRec = async <M extends SkybridgeMode>({
  ...
  logger('Will execute createWithdrawal(%O).', params);

  const { amountIn, nonce } = await runProofOfWork({ ...params, currencyIn: 'sbBTC' });
  ...
  const bridge = getBridgeFor({ ...params, currencyIn: 'sbBTC' });
